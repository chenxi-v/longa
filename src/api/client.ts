...
export async function searchVideos(source: VideoSource, query: string, page: number = 1): Promise<{ list: BfzyVideoItem[], total: number }> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`源 ${source.name} 请求失败: ${response.status}`, proxyUrl.toString());
    ...
    if (data.code === 1) {
      ...
      console.error(`源 ${source.name} 返回错误:`, data.msg);
      throw new Error(data.msg || '搜索失败');
    }
  } catch (error) {
    console.error(`源 ${source.name} 搜索异常:`, error);
    throw error;
  }
}
...
export async function batchGetVideoCovers(
  ...
  const videosWithoutCover = videos.filter(video => {
    const hasCover = video.vod_pic && video.vod_pic.trim() !== '';
    return !hasCover && video.sourceId && video.vod_id;
  });
  
  if (videosWithoutCover.length === 0) {
    videos.forEach(video => {
      if (video.vod_pic && video.vod_id) {
        covers[video.vod_id] = video.vod_pic;
      }
    });
    return covers;
  }
  ...
  for (let i = 0; i < videosWithoutCover.length; i += batchSize) {
    const batch = videosWithoutCover.slice(i, i + batchSize);
    ...
    if (i + batchSize < videosWithoutCover.length) {
  ...
  videos.forEach(video => {
    if (video.vod_id && video.vod_pic && !covers[video.vod_id]) {
      covers[video.vod_id] = video.vod_pic;
    }
  });
...
export function parsePlayUrl(playUrl: string): Episode[] {
  ...
  sourceGroups.forEach((group, groupIndex) => {
    ...
    lines.forEach((line, lineIndex) => {
      ...
      if (parts.length > 1) {
        const label = parts[0] || `播放${episodes.length + 1}`;
